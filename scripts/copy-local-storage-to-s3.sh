...
# keep working after you switch to STORAGE_PROVIDER=s3. Safe to run more than once.
...
exec "$DOCKER" run --rm --network host -v "$PWD/local-storage":/data:ro \
