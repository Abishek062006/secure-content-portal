...
**Live app:** https://secure-content-portal.onrender.com
**Repo:** https://github.com/Abishek062006/secure-content-portal

> First load can take 30–60 seconds — the free-tier instance sleeps after 15 minutes of
...
| Views | Thymeleaf + hand-rolled CSS, same origin | No React/Next split — the app and its views are one Spring Boot process, so the session cookie and the OAuth redirect are same-origin by construction. No CORS, no `SameSite=None`, one deployable |
...
| Hosting | [Render](https://render.com) free web service, Docker | Builds the Dockerfile in the cloud — no local Docker needed |

## Architecture

```mermaid
flowchart TB
    Browser["Browser<br/>(HttpOnly, Secure, SameSite=Lax session cookie)"]

    subgraph Render["Render — single Spring Boot container"]
        direction TB
        Views["Thymeleaf views<br/>role-aware, server-rendered"]
        Sec["Spring Security<br/>Google OIDC → role from our own DB"]
        Admin["Admin controllers<br/>hasRole(ADMIN) + CSRF"]
    ...
    Admin -->|JDBC| Neon
    Stream -->|JDBC| Neon
    Admin -->|S3 API, server-side creds only| Supabase
    Stream -->|S3 API, server-side creds only| Supabase
```

The browser never learns a storage key, never receives a storage credential, and never holds a URL
that outlives its session.

## Running it locally

**Prerequisites:** Java 21, Maven, and the accounts described in [Environment variables](#environment-variables)
(Google OAuth client, Neon database, Supabase bucket).
...
`run-local.sh` sources `.env` and runs `mvn spring-boot:run`. The app starts on
...
|---|---|---|
| App | Render (Docker web service) | 512MB RAM, sleeps after 15 min idle |
...
a dev-time check, not a deploy-time gate.

**The one deployment-specific gotcha worth knowing:** Render terminates TLS upstream of the
container, so without `server.forward-headers-strategy=native` (set in `application-prod.yml`),
Spring builds the OAuth callback URL as `http://` and Google rejects it. This is set correctly here,
but it's the first thing to check if OAuth breaks only in production and not locally.
...
- **HttpOnly, Secure (in production), SameSite=Lax session cookie.** No token is ever stored in
...
| `AccessControlTest` (8 cases) | Anonymous/viewer/admin against every admin route, the delete route, the library, and all three content-delivery endpoints, through the real Spring Security filter chain | Yes |
